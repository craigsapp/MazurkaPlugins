...
// Creation Date: Sat Oct  9 22:41:29 PDT 2010 (copied from MzNevermore)
// Last Modified: Sat Oct  9 22:41:43 PDT 2010
// Filename:      MzPowerSpectrogram.cpp
// URL:           http://sv.mazurka.org.uk/src/MzPowerSpectrogram.cpp
// Documentation: http://sv.mazurka.org.uk/MzPowerSpectrogram
// Syntax:        ANSI99 C++; vamp 1.0 plugin
//
// Description:   Display multiplcation of two spectra of the same
// 		  underlying signal created with different windows.
//

// Defines used in getPluginVersion():
#define P_VER    "201010100"
#define P_NAME   "MzPowerSpectrogram"

#include "MzPowerSpectrogram.h" 
...
///////////////////////////////
//
// MzPowerSpectrogram::MzPowerSpectrogram -- class constructor.
//

MzPowerSpectrogram::MzPowerSpectrogram(float samplerate) : MazurkaPlugin(samplerate) {
...
///////////////////////////////
//
// MzPowerSpectrogram::~MzPowerSpectrogram -- class destructor.
//

MzPowerSpectrogram::~MzPowerSpectrogram() {
   // do nothing
}
...
//////////////////////////////
//
// MzPowerSpectrogram::getParameterDescriptors -- return a list of
...
MzPowerSpectrogram::ParameterList MzPowerSpectrogram::getParameterDescriptors(void) const {
   ...
   pd.name         = "Smaller window size";
   pd.unit         = "samples";
   pd.minValue     = 2.0;
   pd.maxValue     = 5000;
   pd.defaultValue = 500.0;
   pd.isQuantized  = true;
   pd.quantizeStep = 1.0;
   pdlist.push_back(pd);
...
//////////////////////////////
//
// MzPowerSpectrogram::getPreferredStepSize -- overrides the 
//     default value of 0 (no preference) returned in the 
//     inherited plugin class.
//

size_t MzPowerSpectrogram::getPreferredStepSize(void) const {
...
//////////////////////////////
//
// MzPowerSpectrogram::getPreferredBlockSize -- overrides the 
//     default value of 0 (no preference) returned in the 
//     inherited plugin class.
//

size_t MzPowerSpectrogram::getPreferredBlockSize(void) const {
...
std::string MzPowerSpectrogram::getIdentifier(void) const
   { return "mzpowerspectrum"; }

std::string MzPowerSpectrogram::getName(void) const
   { return "PowerSpectrum Spectrogram"; }

std::string MzPowerSpectrogram::getDescription(void) const
   { return "PowerSpectrum Spectrogram"; }

std::string MzPowerSpectrogram::getMaker(void) const
   { return "The Mazurka Project"; }

std::string MzPowerSpectrogram::getCopyright(void) const
   { return "2010 Craig Stuart Sapp"; }

int MzPowerSpectrogram::getPluginVersion(void) const {
...
//////////////////////////////
//
// MzPowerSpectrogram::getInputDomain -- the host application needs
...
MzPowerSpectrogram::InputDomain 
MzPowerSpectrogram::getInputDomain(void) const { 
   return TimeDomain; 
}
...
//////////////////////////////
//
// MzPowerSpectrogram::getOutputDescriptors -- return a list describing
...
MzPowerSpectrogram::OutputList 
MzPowerSpectrogram::getOutputDescriptors(void) const {
...
//////////////////////////////
//
// MzPowerSpectrogram::initialise -- this function is called once
//     before the first call to process().
//

bool MzPowerSpectrogram::initialise(size_t channels, size_t stepsize, 
      size_t blocksize) {
   ...
   if (mz_transformsize < getBlockSize()) {
      std::cerr << "MzPowerSpectrogram::initialize: transform size problem" 
                << std::endl;
      std::cerr << "MzPowerSpectrogram::initialize: transformsize = " 
                << mz_transformsize << std::endl;
      std::cerr << "MzPowerSpectrogram::initialize: blocksize = " 
...
std::cerr << "MzPowerSpectrogram::initialize : window is set to " 
...
//////////////////////////////
//
// MzPowerSpectrogram::process -- This function is called sequentially on the 
...
MzPowerSpectrogram::FeatureSet MzPowerSpectrogram::process(AUDIODATA inputbufs, 
      Vamp::RealTime timestamp) {

   if (getStepSize() <= 0) {
      std::cerr << "ERROR: MzPowerSpectrogram::process: "
                << "MzPowerSpectrogram has not been initialized"
                << std::endl;
      return FeatureSet();
   }
...
//////////////////////////////
//
// MzPowerSpectrogram::getRemainingFeatures -- This function is called
...
MzPowerSpectrogram::FeatureSet MzPowerSpectrogram::getRemainingFeatures(void) {
...
//////////////////////////////
//
// MzPowerSpectrogram::reset -- This function may be called after data processing
...
void MzPowerSpectrogram::reset(void) {
