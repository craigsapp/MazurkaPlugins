...
class MzPowerCurve : public MazurkaPlugin {
   ...
   private: 
      ...
      int mz_overlap;               // sample overlap between successive frames
      int mz_channels;              // number of audio channels in the input

      double mz_powersum;           // for storing the sum for power calc.
      int    mz_framenumber;        // current frame being processed
      std::list<double> sumbuffer;  // storage for efficient power calculations
