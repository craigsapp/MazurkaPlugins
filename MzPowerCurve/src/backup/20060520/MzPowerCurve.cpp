...
// Last Modified: Tue May 16 18:58:36 PDT 2006
...
MzPowerCurve::MzPowerCurve(float samplerate) : Plugin(samplerate) {
   mz_blocksize = 0;
   mz_stepsize  = 0;
   mz_channels  = 0;

   mzp_windowsize = int(0.010*samplerate + 0.5);  // 10 milliseconds by default
   mzp_windowstep = int(0.010*samplerate + 0.5);  // 10 milliseconds by default
   mzp_cutoff     = -50.0;       // cutoff threshold for scaled power slope
   mzp_width      =  10.0;       // threshold width  for scaled power slope
}
...
MzPowerCurve::FeatureSet MzPowerCurve::getRemainingFeatures(void) {
   ...
   double scaling;
   double cutoff = mzp_cutoff;
   double width  = mzp_width;
   for (i=0; i<size-1; i++) {
      scaling = (smoothpower[i] - cutoff)/(width/2.0);
      scaling = 1.0 / (1.0 + pow(2.718281828, -scaling));
      productslope[i] = smoothslope[i] * scaling;
      timeinsec = (getBlockSize() + (2*i+1)*getStepSize())/(2.0*getSrate());
      // check on timinsec value relation smoothpower and adjust
      feature.timestamp = Vamp::RealTime::fromSeconds(timeinsec);
      feature.values.clear();
